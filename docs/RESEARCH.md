...
### exp_011f 残差归因 + 真实场景验证（✅ 关键反转）

真实场景（3s ref + 800 字 target），2×2 因子：NTK(修距离) × 滑窗(除人海，
只屏蔽远处已生成 speech token，稿子/音色锚全保留)：

| 条件 | NTK | 滑窗 | CER | 解读 |
|------|:--:|:--:|:--:|------|
| G_win 门控 | — | W200 | **0.003** | 滑窗无毒✓（masked_calls=13560 生效证明） |
| F0 基线 | — | — | 0.696 | 崩溃复现 |
| F1 | 4× | — | 0.650 | **NTK 在真实场景仅 -0.05！** |
| F2 | — | W200 | 0.719 | 滑窗单独无效 |
| F3 | 4× | W200 | 0.602 | F1−F3=0.048 < 0.05 |

**裁决（按预注册标准）**：
- **B（稀释）否定**：除掉人海无增益（F2≈F0；F3−F1 低于阈值）
- **关键反转：长前缀代理的 70% 恢复无法迁移到真实长文本**（仅 ~7%）

**机制解释（代理 vs 真实的本质差异）**：
- 长前缀代理：稿子只有 150 字（**读稿任务本身在分布内**），只有几何坐标
  越界 → NTK 修几何 → 70% 恢复
- 真实长文本：稿子 765 token，模型需要在长稿上维持"读到哪了"的指针——
  **这个行为策略训练时从没学过**（训练稿 ≤200 字）。NTK 修得了几何表示，
  修不了没学过的行为 → 仅边际改善
- 佐证：F1 的 ASR 从文本 1/3 处开始读且出现循环复读（"觉得孤单…觉得孤单"），
  音频时长 66s→90s——几何修复后模型仍不会"管理长稿指针"

**最终格局**：崩溃 = 几何 OOD（可免训练修，已证）+ **长稿读稿策略缺失
（推理不可修，需长数据微调或切分）**。→ **切分是产品唯一解，Agent 方案
获得完整机制级辩护；NTK 是面试级机制发现而非产品修复。**
