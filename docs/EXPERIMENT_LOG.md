...
## exp_011f — 残差归因 + 真实场景验证（系列收官）

- **日期**：2026-06-12 夜间跑完，06-13 取回（实例满载期 SSH 全程不可达，nohup 落盘）
- **脚本**：`experiments/exp_011_longtext_rootcause/exp5_residual_dilution.py`
- **设计**：真实场景（3s ref + 800 字）2×2：NTK4 × 选择性滑窗（sdpa patch 只屏蔽
  W=200 之外的已生成 speech token，稿子/ref 全保留）；门控 G_win（150字+滑窗）
- **结果**：`results/exp011/exp5_residual_results.json`
- **发现**：
  1. 门控通过（0.003）+ masked_calls 生效证明 → 仪器合格
  2. **B（稀释）否定**：F2≈F0（0.719 vs 0.696）；F3−F1=0.048 低于预注册阈值 0.05
  3. **关键反转**：NTK 在真实长文本仅救 ~7%（0.696→0.650），代理场景的 70% 不迁移
  4. 机制：代理只有几何 OOD（稿短，读稿策略在分布内）；真实场景叠加
     "长稿读稿策略缺失"（训练稿≤200字）——NTK 修几何不修策略
  5. ASR 佐证：F1 从文本 1/3 处起读 + 循环复读，时长 66s→90s
- **工程定论**：切分是产品唯一解；Agent 方案获得完整机制级辩护
- **方法论**：代理实验结论必须在真实场景复核——本系列同时示范了代理的价值
  （干净归因）和边界（不可直接外推）
